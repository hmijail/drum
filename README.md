...
* `dafny_measure`: a wrapper around `dafny measure-complexity` for easier management of the generated logs, by adding to them:
  - The timestamp and arguments used in this verification run
  - Dafny's stdout and stderr, to augment the logs with details that are otherwise missing
  - The file's contents and a hash, to ensure that multiple logs can be meaningfully compared as the code evolves

  Additionally, `dafny_measure` warns when some Dafny toolchain bugs are detected, like when z3 processes are leaked (Dafny issue #[5616](https://github.com/dafny-lang/dafny/issues/5616)).
